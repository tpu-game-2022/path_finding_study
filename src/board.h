...
private:
	...
	void calcCost(const Point target)
	{
		...
		//ゴールまでのコスト推定値を3倍大きくした場合、最短経路が変わってしまうので不採用
		//estimate_ = 3.0 * Point::distance(pos_, target);
		//ゴールまでのコスト推定値を2倍にした場合は最短経路に変化はなかったので採用
		estimate_ = 2.0 * Point::distance(pos_, target);
	}
...
bool static asc(const Mass* o1, const Mass* o2)
{
