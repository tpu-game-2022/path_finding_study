...
private:
	...
	static int getWalkCostWater(Mass& m) { status s = m.getStatus(); return (s == WATER)  ? 3 : 1; }
	static int getWalkCostRoad(Mass& m) { status s = m.getStatus(); return (s ==ROAD) ? 1/3 : 1; }

	void calcConst(const Point target)
	{
		if (WATER)
		{
			steps_ = (pParent_ ? pParent_->steps_ : 0) + getWalkCostWater(*this);
			estimate_ = Point::distance(pos_, target);
		}
		else if (ROAD)
		{
			steps_ = (pParent_ ? pParent_->steps_ : 0) +  getWalkCostRoad(*this);
			estimate_ = Point::distance(pos_, target);
		}
		else
		{
...
public:
	...
	{
		pParent_ = pParent; calcConst(goal);
	}
	Mass* getParent()
	{
		return pParent_;
